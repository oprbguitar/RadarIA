...
function renderCards(){
  ...
  const bvl=state.data.bvl;setCard('bvl',bvl.data?`<div class="mini-table mkt-table"><div class="table-head">Índice <span>Valor</span><span>Var. %</span></div>${bvl.data.map(r=>`<div><b>${r.name}</b><span>${r.price.toLocaleString('es-PE',{maximumFractionDigits:2})}</span><span class="${r.changePercent<0?'down':'up'}">${r.changePercent<0?'▼':'▲'} ${Math.abs(r.changePercent)}%</span></div>`).join('')}</div>${demoNote(bvl)}`:unavailable(bvl));
...
/* ---------- modal de detalle ---------- */
const titles={exchangeRate:'Tipo de cambio · USD/PEN',weather:'Clima',earthquakes:'Sismos',agriculture:'Agro · precios mayoristas',maritime:'Marina · estado del mar',bvl:'Mercados · BVL y referencias internacionales',elPeruano:'Normas · El Peruano'};
const detail={
  ...
  bvl(d){if(!d.data)return unavailable(d);return`<div class="mini-table mkt-table d-table"><div class="table-head">Índice <span>Valor</span><span>Var. %</span></div>${d.data.map(r=>`<div><b>${r.name}</b><span>${r.price.toLocaleString('es-PE',{maximumFractionDigits:2})} ${r.currency||''}</span><span class="${r.changePercent<0?'down':'up'}">${r.changePercent<0?'▼':'▲'} ${Math.abs(r.changePercent)}%</span></div>`).join('')}</div><p class="d-text">Índice bursátil peruano (S&P/BVL Perú General) junto a las principales referencias internacionales, para contexto comparativo. ${d.note||''}. Fuente: <a href="${d.sourceUrl}" target="_blank" rel="noreferrer">${d.sourceName}</a>.</p>`;},
...
/* ---------- fuentes ---------- */
const sourceMeta=[['BCRP','Banco Central de Reserva','bcrp','BCR'],['SENAMHI / OPEN-METEO','Meteorología','weather','☀'],['IGP / USGS','Sismos','earthquakes','IGP'],['MIDAGRI','Desarrollo agrario','agriculture','♧'],['DHN','Hidrografía','maritime','⚓'],['SMV','Mercado de valores','smv','SMV'],['YAHOO FINANCE','BVL + mercados intl.','bvl','BVL'],['EL PERUANO','Diario Oficial','elPeruano','EP']];
...
function localAnswer(text){const t=(text||'').toLowerCase();const d=state.data;
  ...
  if(t.includes('bolsa')||t.includes('bvl')||t.includes('mercado')){const mk=d.bvl?.data;if(mk)return`**Mercados:**\n`+mk.map(r=>`• ${r.name}: ${r.price.toLocaleString('es-PE',{maximumFractionDigits:2})} (${r.changePercent>=0?'▲':'▼'} ${Math.abs(r.changePercent)}%)`).join('\n');}
  const d2=state.data,L=['**Resumen del panel:**'];const e=d2.exchangeRate?.data;if(e)L.push(`• **Dólar:** S/ ${e.sell.toFixed(3)} (venta)`);const w=d2.weather?.data;if(w)L.push(`• **Clima Lima:** ${Math.round(w.temp)}°C, ${weatherLabel(w.weatherCode).toLowerCase()}`);const q=d2.earthquakes?.data;if(q)L.push(`• **Sismo:** M${q.magnitude.toFixed(1)}, ${q.location}`);const m=d2.maritime?.data;if(m)L.push(`• **Mar (Callao):** nivel ${m.level}`);const a=d2.agriculture?.data;if(a)L.push(`• **Agro:** ${a.slice(0,3).map(r=>r.product+' S/'+r.price.toFixed(2)).join(' · ')}`);const mk=d2.bvl?.data;if(mk)L.push(`• **Mercados:** ${mk.slice(0,2).map(r=>r.name+' '+(r.changePercent>=0?'+':'')+r.changePercent+'%').join(' · ')}`);const n=d2.elPeruano?.data;if(n)L.push(`• **Normas:** ${n.length} recientes en El Peruano`);return L.join('\n');}
